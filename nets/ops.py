...
def generalized_box_Ciou(boxes1, boxes2):
    ...
    # 计算交集的左上角坐标和右下角坐标
    inter_x1 = torch.max(x1[:, None], x2)
    inter_y1 = torch.max(y1[:, None], y2)
    inter_x2 = torch.min(x1[:, None] + w1[:, None], x2 + w2)
    inter_y2 = torch.min(y1[:, None] + h1[:, None], y2 + h2)
    ...
    diag_sq = (torch.max(w1[:, None], w2) ** 2) + (torch.max(h1[:, None], h2) ** 2)
    c = center_dist_sq / diag_sq
