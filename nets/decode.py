...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.Line = nn.Linear(d_model, d_model)
        self.act = nn.SiLU
        #self._reset_parameters()
        ...
        self.c = int(d_model * 0.5)
        self.cv1 = nn.Linear(d_model, 2 * self.c)
        self.cv2 = nn.Linear(2 * self.c, d_model)

        self.pos1 = nn.Linear(d_model, d_model)
        self.pos2 = nn.Linear(self.c, int(self.c * 0.5))
        self.softmax = nn.Softmax(-1)
        self.q_scaled = self.c ** -0.5

        self.an_bias = nn.Parameter(torch.zeros(self.num_heads, 300, 300))

    def _reset_parameters(self):
        init.linear_init_(self.linear1)
        init.linear_init_(self.linear2)
...
class Decoder(nn.Module):
    ...
    def _get_encoder_input(self, feats):
        ...
        feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]
    ...
    def _generate_anchors(self,
        ...
        #anchors = torch.where(valid_mask, anchors, torch.tensor(float('inf')).to(device))
