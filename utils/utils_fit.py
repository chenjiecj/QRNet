...
def fit_one_epoch(model_train, model, ema, detr_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    loss        = 0
    val_loss    = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #with autocast():
            with torch.autocast(device_type=str('cuda'), cache_enabled=True):
                outputs = model_train(images, targets)

            with torch.autocast(device_type=str('cuda'), enabled=False):
